...
# # Regex to parse DMS: allow N, S, E, or W, e.g. “E 010° 55' 45.05"
# DMS_RE = re.compile(
#     r"""^\s*([NSEW])\s*    # Hemisphere: N, S, E or W
#         (\d+)[°\s]+       # Degrees
#         (\d+)[\'\s]+      # Minutes
#         ([0-9.]+)"?       # Seconds
#         \s*$""",
...
#             raise ValueError(f"Row {i+1} has {len(vals)} values; expected {len(cols)}")
#         records.append(vals)
...
#         # Dup check on ID + normalized lat/lon
#         cursor.execute(f"""
#             SELECT 1 FROM "{table}"
#              WHERE NDBIdentifier = ?
#                AND NDBLatitude   = ?
...
#         # Decimal WGS84 coords
#         lat = dms_to_decimal(raw_lat) or None
#         lon = dms_to_decimal(raw_lon) or None
